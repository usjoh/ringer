...
| `check_timeout_s` | Per-task kill timer for the `check` (default 60). Raise it for gates that build, install, or run a real test suite — a check killed at the default is reported as TIMEOUT, which reads as a model failure when it is only a slow gate |
...
OpenCode ships no OS sandbox, so the engine's `bin` points at an absolute path to `engines/opencode-sandboxed.sh` (ringer does not resolve engine bins relative to the repo): a macOS Seatbelt wrapper that leaves network and reads open but confines writes to the task dir, a per-run scratch dir (wired as the agent's `TMPDIR`/`XDG_CACHE_HOME`/`XDG_DATA_HOME`), and OpenCode's own state/config dirs. Pointing `XDG_DATA_HOME` at that per-run dir gives every worker a **private session store**: OpenCode keeps its session state in a single unbounded SQLite file, and workers sharing it deadlock — the losers die with `database is locked` before the model runs, which Ringer can only record as an ordinary FAIL. Since every non-Codex model routes through this engine, that contention penalised exactly the cheap tier you fan out widest. Credentials live inside that directory, so the wrapper seeds the private root by **copying** `auth.json` (never symlinking — a link resolves straight back to the shared file). `--no-sandbox` deliberately keeps the shared store: that path `exec`s, so the scratch dir's cleanup trap would never fire. It is fine for a lone full-access task, not for a fan-out. Its `--dangerously-skip-permissions` flag only silences OpenCode's interactive prompts; Seatbelt is the actual containment. Task paths reach the profile as `sandbox-exec -D` parameters rather than string interpolation, so a task dir with quotes or parens can't inject sandbox rules. `--no-sandbox` is wired as the engine's `full_access_args`, so ringer's `allow_full_access` gate still governs escapes. Non-macOS installs need their own sandbox (or full-access mode).
