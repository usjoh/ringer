...
- 2026-07-28 (meridian, harness-audit-remediation): gpt-5.6-sol medium ×6 code-fix/test-hardening in worktrees + low ×1 — 4/7 first-try, 5/7 in-run (~33-36k tokens each; the fifth lane recovered on a legit retry at 119k). The 2 FAILs are ORCHESTRATOR spec defects, not model: both lanes were spec'd to create `.codex/` inside the worktree, and the Codex CLI sandbox HARD-BLOCKS writes to `.codex/` in its own workspace (`mkdir -p .codex` → Operation not permitted; apply_patch rejects the path). Worker conduct exemplary on both: one adapted by writing validated TOMLs to the check's own export dir and proving them with the env-override test (5 passed) — orchestrator-verified PASS from surviving worktree; the other shipped its test, documented the blocker with exact evidence, and refused to scatter files outside its ownership list. CAUTION for the eval log: these two rows classify as `model` even under failure attribution — only this note corrects them. Lessons: never spec a codex worker to write `.codex/` inside its workspace — use a projection dir outside the worktree and point the test at it via env; and a skip-aware test needs a vacuous-skip guard in the check (one lane's skip surfaced as pytest exit 5, the other's as exit 0 — same design hole, two exit codes).
...
## GPT-5.6 Sol — 2026-08-01 (cttc-journal-upload, ms-20260801-0554)
